...
class AccountingAPI(http.Controller):
    ...
    @validate_token
    @validate_jwt
    @http.route('/accounting/post_journal_entry', type="http", auth="none", methods=["POST"], csrf=False)
    def post_journal_entries(self, **payload):
        print("---")
        ref = payload.get("reference", False)
        journal_id = payload.get("journal_id", False)
        line_ids = payload.get("items", False)
        journal_data = {
            "ref": ref,
    ...
    @validate_token
    @validate_jwt
    @http.route('/mobile/student_list', type="http", auth="none", methods=["get"], csrf=False)
    def student_list(self, uid, **payload):
        if not payload:
            payload = json.loads(request.httprequest.data)
        kw = payload.get("kw", False)
        try:
            partners = get_table_model('res.partner').search([('name', 'ilike', kw)], limit=50)
            partner_list = []
            ...
            return valid_response_http(data=partner_list, status=200)
        except:
            message = _("Error during get data model. Please contact to administrator.")
        return invalid_response_http(type="Student List", message=message, status=403)

    @validate_token
    @validate_jwt
    @http.route('/accounting/post_invoice', type="http", auth="none", methods=["POST"], csrf=False)
    def post_invoice(self, uid, **payload):
        print("---")
        if not payload:
            payload = json.loads(request.httprequest.data)
        journal_id = payload.get("journal_id", False)
        student_id = int(payload.get("student_id", False))
        user = get_table_model('res.users').browse(uid)
        lines = payload.get("lines", [])

        if not journal_id:
            journal_id = request.env['account.journal'].sudo().search([('type', 'in', ('bank', 'cash'))], limit=1)
        partner_id = get_table_model('res.partner').browse(student_id)
        vals = {
            'payment_reference': partner_id.name,
            'invoice_origin': "CUS",
            'journal_id': journal_id,
        ...
        move_id = get_table_model('school.invoice').create_invoice(vals)
